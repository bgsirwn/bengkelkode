<?
class PengeluaranTable extends KoneksiDatabase{
	var $id;
	var $user_id;
	var $name;
	var $amount;
	...
	function getByUserId($user_id){
		...
		$query = $koneksi->prepare("SELECT id,user_id, name, amount, description, date,
		 ...
		 EXTRACT(MINUTE from date) as minute,
		 EXTRACT(SECOND from date) as second
		 FROM pengeluaran WHERE user_id=:user_id ORDER BY id DESC");
	...
	function saveRow(){
		$koneksi = new PDO('mysql:host='.$this->host.';dbname='.$this->database,$this->usr,$this->pass);
		$query = $koneksi->prepare("INSERT INTO pengeluaran (id, user_id, name, amount, description, date) VALUES(null,:user_id,:name,:amount,:description,:date)");
